...
if (isset($_POST['username']) && isset($_POST['password']) && isset($_POST['mail'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM users WHERE user_name = ? OR user_password = ? OR mail = ?");
    $stmt->execute([$input_username, $input_password, $input_mail]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($user) {
        echo "Bu kullanıcı adı ya da e-posta adresi zaten alınmış!";
    } else {
        // Şifreyi hashleyerek veritabanına ekle
        $hashed_password = password_hash($input_password, PASSWORD_DEFAULT);

        // Misafir mi değil mi kontrol et (Misafir kayıt yapacaksa, is_guest = 1 olur)
        $is_guest = isset($_POST['is_guest']) && $_POST['is_guest'] == '1' ? 1 : 0;

        // Kullanıcıyı veritabanına ekle
        $stmt = $pdo->prepare("INSERT INTO users (user_name, user_password, mail, is_guest) VALUES (?, ?, ?, ?)");
...
<div class="form-container">
    ...
    <form action="register.php" method="POST">
        ...
        <!-- Misafir olarak kayıt olmak için gizli alan -->
        <input type="hidden" name="is_guest" value="0"> <!-- 0 ise misafir değil, 1 ise misafir -->
