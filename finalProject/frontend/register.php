...
if (isset($_POST['username']) && isset($_POST['password']) && isset($_POST['mail'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM users WHERE user_name = ? OR mail = ?");
    $stmt->execute([$input_username, $input_mail]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($user) {
        echo "This username or email address is already taken!";
    } else {
        // Hash the password before saving
        $hashed_password = password_hash($input_password, PASSWORD_DEFAULT);

        // Set is_guest to 0 and expiry_time to 0
        $is_guest = 0;
  ...
  <main>    
    <div class="container">
      ...
      <input type="text" name="username" id="kadi" placeholder="kullanıcı adınız..." style="width: 100%;" required> <i class="fa-solid fa-user"></i></div>
       <div class="email-icon"> <input type="email" name="mail" id="email" placeholder="mail adresiniz..." style="width: 100%;" required>
