...
$shareLink = ''; // Paylaşılabilir linki tutmak için
...
if (isset($_FILES['file'])) {
    ...
    if ($fileError === 0) {
        ...
        // Dosyanın güvenli bir şekilde yüklenmesi
        $newFileName = uniqid('', true) . '.' . $fileExt;
        $fileDestination = 'uploads/' . $newFileName;
        if (move_uploaded_file($fileTmpName, $fileDestination)) {
            // Dosya başarıyla yüklendiyse, expire_time hesaplayıp veritabanına kaydediyoruz
            $currentTimestamp = time(); // Şu anki timestamp
            $expireTimestamp = $currentTimestamp + 86400; // 24 saat (86400 saniye) ekleyerek expire_time'ı hesaplıyoruz
...
$files = [];
$sql = "SELECT * FROM files";  // files tablosundaki tüm dosyaları alıyoruz
$stmt = $pdo->query($sql);

// Veritabanından gelen veriyi $files dizisine atıyoruz
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $files[] = $row;
}

// 24 saatten fazla süresi geçmiş dosyaları silme
$currentTimestamp = time(); // Şu anki timestamp

// expire_time'ı kontrol edip 24 saatten fazla geçmiş dosyaları silme
...
?>
    ...
    .promotion {
            ...
            .typing-effect {
                display: inline-block;
                border-right: 3px solid #333; /* İmleç */
                padding-right: 5px;
                white-space: nowrap;
                overflow: hidden;
            }
    ...
    padding: 15px; 
    box-shadow: 9px 15px 1px 0px black;  /* Sabit gölge */
    transition: transform 0.4s ease, box-shadow 0.3s ease;  /* Hem transform hem de box-shadow için geçiş */
    }
    .cards h3{
    color: #2C3E50;
    }
    ...
    transform: translateY(-10px);  /* Kartı 10px yukarı kaydırır */
    box-shadow: 9px 25px 1px 0px black;  /* Gölgeyi büyütüyoruz */
    }
    ...
    .price-card {
    background-color: #fff; /* Diğer kartlardan farklı olarak beyaz arka plan */
    padding: 25px 20px; /* Daha geniş iç padding */
    width: 100%;
    box-shadow: 0 10px 30px rgba(0, 0, 0, 0.15); /* Derin gölge */
    border-radius: 12px; /* Keskin köşeler veya yuvarlak köşeler için değiştirebilirsiniz */
    transition: all 0.3s ease; /* Hover animasyonu için geçiş */
    text-align: center;
    border: 1px solid #000000; /* Hafif sınır ekleyebiliriz */
    }
    ...
    box-shadow: 0 15px 50px rgba(0, 0, 0, 0.2); /* Daha belirgin gölge */ 
    }
    .premium-price .price-card:hover h4 {
    color: #fff; /* Hoverda başlık rengi beyaz olur */
    }

    /* Diğer Kartlar */
    .price-card:nth-child(1) {
    background-color: #66bcf1; /* İlk kart için daha soft bir renk */
    border-radius: 15px;
    }

    /* Premium Fiyatlandırma Kartı için vurgulama */
    .price-card:nth-child(2) {
    background-color: #66bcf1; /* Orta kart için farklı bir renk tonu */
    box-shadow: 0 6px 15px rgba(0, 0, 0, 0.1); /* Daha hafif gölge */
    }


    .price-card:nth-child(3) {
    background-color: #66bcf1; /* Üçüncü kart için farklı bir renk tonu */
    border-radius: 15px;
    ...
    <header>
        ...
        <a href="index.php"><img src="logo.png" alt="" style="width: 80px;"></a>
        <ul> 
    ...
    <div align="center" class="promotion">
        <h1 style="font-size: 36px;">DOSYALARINIZI GÜVENLE <span id="word"></span></h1>
            ...
            <img src="upload.png" alt=""  width="200">
        ...
        <div class="cards">
        <div class="card1">
            <i class="fa fa-share-alt" style="font-size: 48px;  margin-left: 95px;"></i>
    ...
    <script>
// "YÜKLEYİN", "PAYLAŞIN", "YÖNETİN" kelimeleri arasında geçiş yapacak
...
function typeLetter() {
    if (letterIndex < currentWord.length) {
        ...
        letterIndex++;
        setTimeout(typeLetter, 100); // Harf arası gecikme
    } else {
        setTimeout(() => {
            // Bir kelime bittiğinde, kısa bir süre sonra yeni kelimeye geç
            index = (index + 1) % words.length;
            currentWord = words[index];
            letterIndex = 0;
            wordElement.textContent = ''; // Eski kelimeyi sil
            typeLetter(); // Yeni kelimeyi başlat
        }, 1500); // Kelimeyi tamamladıktan sonra biraz bekle
    }
}

// Başlangıçta ilk kelimeyi yazmaya başla
currentWord = words[index];
typeLetter();  

// Dosya inputu tıklama işlemi
...
function uploadFile() {
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            ...
            location.reload(); // Yükleme başarılı olursa sayfayı yenile
        } else {
            alert("Dosya yükleme sırasında bir hata oluştu.");
        }
    };

    xhr.send(formData); // Form verisini gönder
}

// Sürükle-bırak dosya inputu işlemi
...
// AJAX ile dosya silme fonksiyonu
    ...
    if (confirm('Emin misiniz? Bu dosya kalıcı olarak silinecek.')) {
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                location.reload(); // Sayfayı yenileyerek dosya listesine güncel hali yansıtır
            } else {
